...
class RobotRunnerConfig:
    name:                       str             = "rl4greenros_composition_obstacles-simtime"
    ...
    nav2_res_profile: ProcessResProfiler
    ...
    def __init__(self):
        ...
        if not rl4greenros_path:
            ...
            self.cpu_mem_profiler = CPUMemProfiler('nav2_composition_container','cpu-mem.csv')
            self.nav2_res_profile = ProcessResProfiler('component_container_isolated', 'nav2')
        ...
        self.containers = ['gazebo', 'nav2_composition', 'rviz', 'robot-runner']
    ...
    def create_run_table(self) -> List[Dict]:
        ...
        run_table = RunTableModel(
           factors = [
                FactorModel("round", range(1,5)),
                FactorModel("configuration", range(1,10)),
                FactorModel("position_goal", [1,2,3,4,5]),
                FactorModel("number_obstacles", [1,2])
            ] 
        )
    ...
    def stop_measurement(self, context: RobotRunnerContext) -> None:
        ...
        dest_rapl_file = f"~/Documents/experiments/{self.name}/{run_id}/"
        ...
        command_rapl = f"cp {rl4greenros_path}/nav2*.csv {dest_rapl_file}"
        ...
        command_rm_files = f"rm {rl4greenros_path}/performance.csv"
        subprocess.run(command_rm_files, shell=True)
