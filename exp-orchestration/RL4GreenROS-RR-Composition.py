...
class RobotRunnerConfig:
    name:                       str             = "rl4greenros_composition_2"
    ...
    def __init__(self):
        ...
        if not rl4greenros_path:
            ...
            self.cpu_mem_profiler = CPUMemProfiler('nav2_composition_container','cpu_mem.csv')
    ...
    def create_run_table(self) -> List[Dict]:
        ...
        run_table = RunTableModel(
            factors = [
                FactorModel("round", range(1,3)),
                FactorModel("configuration", range(1,25)),
                FactorModel("position_goal", [2]),
                FactorModel("number_obstacles", [0,1])
            ] 
        )
    ...
    def stop_run(self, context: RobotRunnerContext) -> None:
        ...
        self.docker_runner.clean_containers(self.containers)
        command_cleaning = "./clean-container.sh"
